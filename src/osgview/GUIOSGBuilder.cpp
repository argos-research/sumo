...
/// @date    19.01.2012
/// @version $Id: GUIOSGBuilder.cpp 19838 2016-01-28 20:27:42Z behrisch $
///
...
GUIOSGBuilder::buildOSGScene(osg::Node* const tlg, osg::Node* const tly, osg::Node* const tlr, osg::Node* const tlu) {
    ...
    // build junctions
    for (unsigned int index = 0; index < net->myJunctionWrapper.size(); ++index) {
...
GUIOSGBuilder::buildOSGEdgeGeometry(const MSEdge& edge,
    ...
    for (std::vector<MSLane*>::const_iterator j = lanes.begin(); j != lanes.end(); ++j) {
        ...
        int index = 0;
        for (unsigned int k = 0; k < rshape.size(); ++k, ++index) {
