...
/// @date    30.05.2012
/// @version $Id: TraCIAPI.cpp 20433 2016-04-13 08:00:14Z behrisch $
///
...
void
TraCIAPI::VehicleScope::reroute(const std::string& typeID) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_COMPOUND);
  content.writeInt(0);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, CMD_REROUTE_TRAVELTIME, typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
