...
@date    2011-03-16
@version $Id: trafficlights.py 19662 2015-12-23 23:20:53Z behrisch $
...
def subscribeContext(tlsID, domain, dist, varIDs=(tc.TL_CURRENT_PHASE,), begin=0, end=2**31 - 1):
    traci._subscribeContext(
        tc.CMD_SUBSCRIBE_TL_CONTEXT, begin, end, tlsID, domain, dist, varIDs)
...
def setCompleteRedYellowGreenDefinition(tlsID, tls):
    ...
    # programID
    traci._message.packString(tls._subID)
    ...
    for p in tls._phases:
        ...
        traci._message.packString(p._phaseDef)
    traci._sendExact()


getParameter, setParameter = traci.getParameterAccessors(
    tc.CMD_GET_TL_VARIABLE, tc.CMD_SET_TL_VARIABLE)
