...
@date    2011-03-17
@version $Id: edge.py 19649 2015-12-17 21:05:20Z behrisch $
...
def _getUniversal(varID, edgeID):
    result = traci._sendReadOneStringCmd(
        tc.CMD_GET_EDGE_VARIABLE, varID, edgeID)
...
def subscribeContext(edgeID, domain, dist, varIDs=(tc.LAST_STEP_VEHICLE_NUMBER,), begin=0, end=2**31 - 1):
    traci._subscribeContext(
        tc.CMD_SUBSCRIBE_EDGE_CONTEXT, begin, end, edgeID, domain, dist, varIDs)
...
getParameter, setParameter = traci.getParameterAccessors(
    tc.CMD_GET_EDGE_VARIABLE, tc.CMD_SET_EDGE_VARIABLE)
