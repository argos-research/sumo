...
@date    2011-03-09
@version $Id: vehicle.py 19662 2015-12-23 23:20:53Z behrisch $
...
def setRoute(vehID, edgeList):
    ...
    traci._message.packStringList(edgeList)
    traci._sendExact()
...
def setAdaptedTraveltime(vehID, begTime, endTime, edgeID, time):
    ...
    traci._message.string += struct.pack("!BiBiBi", tc.TYPE_COMPOUND, 4, tc.TYPE_INTEGER, begTime,
...
def add(vehID, routeID, depart=DEPART_NOW, pos=0, speed=0, lane=0, typeID="DEFAULT_VEHTYPE"):
    ...
    traci._message.packString(typeID)
