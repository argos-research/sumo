...
@date    2009-06-30
@version $Id: main.py 22608 2017-01-17 06:28:54Z behrisch $
...
for ini in range(1, 5):
    ...
    setTLType(
        os.path.join('sumoConfig', 'one_intersection', 'cross.net.xml'), tlType)

    if (tlType == 'static'):
        satFlow = getSaturationFlow()

    if (tlType == 'actuated'):
        setPhaseMinMax(os.path.join('sumoConfig', 'one_intersection',
                                    'cross.net.xml'), phaseMinWE, phaseMaxWE, phaseMinNS, phaseMaxNS)
    ...
    for i in range(K):
        for vehphWE in vehphWEA:
            for vehphNS in vehphNSA:
                ...
                if (tlType == 'static'):
                    [greenWE, greenNS] = [
                        int(round(greenWE)), int(round(greenNS))]
                else:
