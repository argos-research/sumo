...
@date    2008-07-26
@version $Id: readPlot.py 22608 2017-01-17 06:28:54Z behrisch $
...
if load == XML:
    ...
    filePath = path.main + \
        "/fcdQualitaet/output/plotData/prozFCD_vtypeNurBefahreneKanten"
showVal = RELSPEED
# showVal=EDGENO
...
def fetchData():
    ...
    def readXML(line):
        ...
        if words[0].find("period=") != -1:
            ...
            valueDict.setdefault(period, {}).setdefault(
                quota, []).append(value)
            # get the used quotas
            if firstFile and len(xt) == 1:
                yt.append(quota)
    ...
    for period in xt:
        ...
        for quota in yt:
            ...
            valueDict[period][quota] = sum(
                valueDict[period][quota]) / len(valueDict[period][quota])
            # add avg value
