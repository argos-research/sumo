...
@date    2008-04-15
@version $Id: FetchData.py 22608 2017-01-17 06:28:54Z behrisch $
...
simFcdDict = {}
vtypeDict = {}
# if True edges with no computed traffic in the net will be ignored and
# not pushed in the analysis file.
withoutEmptyEdges = True
    ...
    for line in inputFile:
        if line.find("<timestep") != -1:
            ...
            if words[1] in vtypeDict:
                # Veh_ID            time  edge             lat       lon
                # speed form m/s in km/h
                vtypeDict[words[1]].append(
                    (time, words[3][:-2], words[11], words[13], str(int(round(float(words[15]) * 3.6)))))
            else:
                # Veh_ID      time  edge             lat       lon       speed
                # form m/s in km/h
                vtypeDict[words[1]] = [
                    (time, words[3][:-2], words[11], words[13], str(int(round(float(words[15]) * 3.6))))]
